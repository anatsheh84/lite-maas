  ...
  private async makeRequest<T>(
    ...
    for (let attempt = 1; attempt <= this.config.retryAttempts; attempt++) {
      try {
        ...
        // Bug fix: Do not retry 404 errors — they are permanent (resource not found),
        // not transient. Retrying wastes time and causes isLiteLLM404Error() checks
        // in callers (e.g. getUserInfo) to fail because the error identity is preserved
        // but the retry delay adds unnecessary latency before propagation.
